...
//Receive timeouts for regular operation and "oh-shit" caching mode
#define RCV_TIMEO_NORM (200)
#define RCV_TIMEO_CACHE (10) //may need tweeking
	...
	private:
		...
		void _stream_task(ADS* ads) {
			...
			while(1) {
				...
				int num_loaded = 0; //this is for partially cached packets

				// JCR (regarding recieve timeout): here we go. Have to pass optval in this fucky manner for no particularly good reason(?):
				int rcvTimeo = RCV_TIMEO_NORM; //timeout in MS
				lwip_setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (void*)&rcvTimeo, sizeof(rcvTimeo));
				...
				while(1) 
				{
					...
					if (!mCachingModeOn)
					{
						...
						/*JCROPT
						  if (rcvTimeo != RCV_TIMEO_NORM) { //rcvTimeo should be large enough to handle minor fuckups but timeout for major ones like complete dropouts s.t. we can start caching procedure
						// JCR:, here we go. Have to pass optval in this fucky manner for no particularly good reason(?):
						rcvTimeo = RCV_TIMEO_NORM; //timeout in MS
						lwip_setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (void*)&rcvTimeo, sizeof(rcvTimeo));
						}
						 */
						// blocking (w/ timeout) read for ACK on currently buffered packet
						...
						//So this will be reliant on this else block being run relatively quickly; 
						//JCROPT The first conditional is present so that we won't keep caching if the queue size is low - there's no reason to write to flash unnecesarily
						//We just dump the whole queue repeatedly to the disk until sending catches up
						//TODO Integrate JCROPT which only dumps as much of the queue as is needed
						if (ads->cacheSamples(storageManager) < 0) {
							//JCROPT if ((ads->getQueueSize() > 120 ) && (ads->cacheSamples() < 0)){
						...
						/*JCROPT			if (rcvTimeo != RCV_TIMEO_CACHE) { //rcvTimeo here should be short enough that we can cache at least 8 samples (240B) which is one call to ads->cacheSamples by the time 8 new samples are ready
						// JCR: here we go. Have to pass optval in this fucky manner for no particularly good reason(?):
						rcvTimeo = RCV_TIMEO_CACHE; //timeout in MS
						lwip_setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (void*)&rcvTimeo, sizeof(rcvTimeo));
						}
						 */
						// Blocking read for ACK on currently buffered packet | JCR - timeout implemented
						recvfrom(s, inbuf, inbuf_size, 0, res.ai_addr, &(res.ai_addrlen)); 
						if (inbuf[0] == outbuf[0])
						{
							// It's a proper ACK, move to next packet
							...
							num_loaded = ads->loadCachedSamplesToPacket(storageManager, outbuf);
							if (num_loaded < 56){ 
